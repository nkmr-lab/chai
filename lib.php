...
function chai_generate_image(string $prompt, ?string $size = null): array {
    ...
    $payload = ['model' => $ig['model'], 'prompt' => $prompt, 'size' => $size ?: ($ig['size'] ?? '1024x1024'), 'n' => 1];
    if (!empty($ig['quality'])) $payload['quality'] = $ig['quality'];
...
function chai_edit_image(string $bin, string $mime, string $prompt, ?string $size = null): array {
    ...
    curl_setopt_array($ch, [
        CURLOPT_POST => true,
        CURLOPT_HTTPHEADER => ['Authorization: Bearer ' . $oa['api_key']],
        CURLOPT_POSTFIELDS => ['model' => $ig['model'], 'prompt' => $prompt, 'size' => $size ?: ($ig['size'] ?? '1024x1024'),
                               'image' => new CURLFile($tmp, $mime, 'image.' . $ext)],
        CURLOPT_RETURNTRANSFER => true, CURLOPT_TIMEOUT => 180,
    ]);
    $res = curl_exec($ch); $code = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE); curl_close($ch); @unlink($tmp);
    $j = json_decode((string)$res, true);
    if ($code !== 200 || empty($j['data'][0])) return ['ok' => false, 'error' => $j['error']['message'] ?? "編集失敗（{$code}）"];
    $row = $j['data'][0];
    $out = !empty($row['b64_json']) ? base64_decode($row['b64_json']) : (!empty($row['url']) ? @file_get_contents($row['url']) : null);
    if (!$out) return ['ok' => false, 'error' => '編集画像の取得失敗'];
    $fname = 'edit_' . bin2hex(random_bytes(8)) . '.png';
    $dir = __DIR__ . '/media'; if (!is_dir($dir)) @mkdir($dir, 0775, true);
    file_put_contents($dir . '/' . $fname, $out);
    return ['ok' => true, 'url' => '/media/' . $fname, 'bin' => $out];
}
